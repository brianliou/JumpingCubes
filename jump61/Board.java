...
abstract class Board {
    ...
    /** 2D array which holds the value of each square. */
    protected String[][] _squares;
    ...
    boolean isLegal(Color player, int r, int c) {
        return exists(r, c) && isLegal(player)
            && (player == color(r, c) || color(r, c) == Color.WHITE);
    }
    ...
    boolean isLegal(Color player, int n) {
        return exists(n) && isLegal(player)
            && (player == color(n) || color(n) == Color.WHITE);
    }
    ...
    final Color getWinner() {
        int sqtotal = size() * size();
        if (numOfColor(Color.RED) == sqtotal) {
            return Color.RED;
        } else if (numOfColor(Color.BLUE) == sqtotal) {
            return Color.BLUE;
        } else {
            return null;
        }
    ...
    public String toString() {
        ...
        for (int i = 0; i < size(); i += 1) {
            for (int j = 0; j < size(); j += 1) {
                if (j == 0) {
                    out.format("    " + _squares[i][j] + " ");
                } else if (j == size() - 1) {
                    out.format(_squares[i][j] + "%n");
                } else {
                    out.format(_squares[i][j] + " ");
                }
            }
    ...
    int neighbors(int r, int c) {
        if ((r == 1 && c == 1) || (r == 1 && c == size())
            || (r == size() && c == 1) || (r == size() && c == size())) {
            return 2;
        } else if (r == 1 || c == 1 || r == size() || c == size()) {
            return 3;
        } else {
            return 4;
        }
    ...
    int neighbors(int n) {
        if ((n == 0) || (n == size() - 1) || (n == size() * size() - 1)
            || (n == size() * size() - size())) {
            return 2;
        } else if ((row(n) == size()) || (row(n) == 1)
                || (col(n) == 1) || (col(n) == size())) {
            return 3;
        } else {
            return 4;
